...
namespace Parking.WebAPI.Test.Controllers
{
    public class CustomersControllerTest
    {
        ...
        public async Task CustomersController_Post_ShouldReturnCreatedCustomer()
        {
            ...
            mockCustomerService.Setup(service => service.AddAsync(newCustomerDTO)).ReturnsAsync(newCustomerDTO);

            var controller = new CustomersController(mockCustomerService.Object);

            var result = await controller.Post(newCustomerDTO);
            ...
            Assert.Equal(newCustomerDTO, resultCustomer);
        }
        ...
        public async Task CustomersController_Put_ShouldReturnUpdatedCustomer()
        {
            ...
            var updateCustomerDTO = new CustomerDTO
            {
                Id = customerId,
                Name = "Pedro dos Santos",
                BirthDate = DateOnly.ParseExact("18-04-1997", "dd-MM-yyyy"),
            ...
            mockCustomerService.Setup(service => service.UpdateAsync(updateCustomerDTO)).ReturnsAsync(updateCustomerDTO);

            var controller = new CustomersController(mockCustomerService.Object);

            var result = await controller.Put(customerId, updateCustomerDTO);

            var noContentResult = Assert.IsType<NoContentResult>(result);
            Assert.Equal(StatusCodes.Status204NoContent, noContentResult.StatusCode);

            mockCustomerService.Verify(service => service.UpdateAsync(updateCustomerDTO), Times.Once);
        }
